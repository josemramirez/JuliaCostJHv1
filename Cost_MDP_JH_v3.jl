...
begin
	...
	md"""
	# Markov Decision Processes for Jasper Health 2022.
	##### _Decision Making Under Uncertainty with POMDPs.

	-- Jose Ramirez (Algorithm Department)
	"""
end
...
md"""
## Breast Cancer Problem (MDP)
In the _Breas Cancer_ problem, an _agent_ moves around a grid attempting to collect as much reward as possible, trying to avoid negative rewards.
"""

# ╔═╡ e2a84ebf-a259-43c1-b512-f6c6b6e02d14
md"""
### Environment Parameters (setup)
First we set some parameters that help us define the Breast Cancer environment (these could be global variables, but we choose to consolidate them to a single `GridWorldParams` structure).
...
A state $s$ in the Grid World problem is a discrete $(x,y)$ value in a $5\times5$ grid.
"""
...
# ╔═╡ b760c4da-2670-4864-95a9-19b342612fe0
[x for x in 1:3]
...
#	end
	...
	if s == State(1,cSt)
		return nRew
	elseif s == State(2,cSt)
		return nRew
	elseif s == State(3,cSt)
		return nRew
	elseif s == State(4,cSt)
		return nRew
	elseif s == State(5,cSt)
		return nRew
	else
		return nRew
	end
...
function T(s::State, a::Action)
	...
	if s == State(1,nage)
		p_transition = 0.54
	elseif s == State(2,nage)
		p_transition = 0.46
	elseif s == State(3,nage)
		p_transition = 0.44
	elseif s == State(4,nage)
		p_transition = 0.12
	# Pasing to death is weird!
	elseif s == State(5,nage)
		p_transition = 0.0001
	# -----------------------------------------
	end
...
md"""
 $(x,y)$ = $(@bind state_x NumberField(1:5, default=2)) | $(@bind state_y NumberField(1:4, default=3))
"""
...
function plot_grid_world(mdp::MDP,
    ...
    title!("Breast Cancer Policy Plot$extra_title")
...
# ╔═╡ 2a438be3-89aa-42a3-b273-0a43c4aea372
steps[2]
...
begin
	grid_plot = render(mdp, policy, 10; outline=false, outline_state=sᵣ)
